import require$$1$2, { ipcMain as ipcMain$1, app as app$1, BrowserWindow } from "electron";
...
ipcMain$1.handle("store-save", (_event, key, value) => {
  store.set(key, value);
});
ipcMain$1.handle("store-load", (_event, key) => {
...
ipcMain$1.handle("worker-start", (event, workerData) => {
  if (activeWorker) {
    console.log("Worker already running");
    return;
  }
  const workerPath = path$6.join(__dirname$1, "emailWorker.js");
  ...
  activeWorker.on("exit", (code2) => {
    event.sender.send("worker-exit", { code: code2 });
    activeWorker = null;
  });
});
ipcMain$1.handle("worker-stop", () => {
  if (activeWorker) {
    activeWorker.postMessage({ action: false });
    setTimeout(() => {
      activeWorker == null ? void 0 : activeWorker.terminate();
      activeWorker = null;
    }, 500);
  }
});
ipcMain$1.handle("dialog-open", async (_event, options) => {
  const { dialog } = await import("electron");
  return dialog.showOpenDialog(win, options);
});
ipcMain$1.handle("dialog-save", async (_event, options) => {
  const { dialog } = await import("electron");
  return dialog.showSaveDialog(win, options);
});
ipcMain$1.handle("dialog-message", async (_event, options) => {
  const { dialog } = await import("electron");
